...
import { addOrUpdateClaudeServer, removeClaudeServer } from "./cli.js";
    ...
    for (const step of plan) {
      try {
        if (step.action === "remove") {
          ...
          if (r.code === 0) removed++; else failed++;
        } else {
          const r = await addOrUpdateClaudeServer(step.name, step.json!, scope, cwd);
          if (r.code === 0) {
