...
describe("kc doctor", () => {
    ...
    try {
      ...
      vi.doMock("../src/lib/adapters/registry.ts", () => {
        const adapter = {
          ...
          readUser: async () => ({ source: join(dir, "user.json"), mcpServers: { b: { type: "stdio", command: "echo" } } }),
        } as const;
        return { getAdapter: async () => adapter };
      });
      await writeFile(join(dir, "user.json"), JSON.stringify({ mcpServers: { b: { type: "stdio", command: "echo" } } }), "utf8");

      const { registerDoctorCommand } = await import("../src/commands/doctor.ts");
      const program = new Command();
      registerDoctorCommand(program);
      const logs: string[] = [];
      const spy = vi.spyOn(console, "log").mockImplementation((s: unknown) => { if (typeof s === "string") logs.push(s); });
  ...
  it("reports error when CLI missing (and shows summary)", async () => {
    ...
    try {
      await writeFile(join(dir, ".mcp.json"), JSON.stringify({ mcpServers: { c: { type: "http", url: "https://proj" } } }), "utf8");
...
describe("doctor local overrides", () => {
  it("classifies last local run as localOverrides (ok)", async () => {
    ...
    try {
      ...
      await writeFile(join(dir, ".katacut", "state.json"), JSON.stringify({ version: "1", runs: [ { at: new Date().toISOString(), client: "claude-code", requestedScope: "project", realizedScope: "project", mode: "native", intent: "local", result: { added:0,updated:0,removed:1,failed:0 }, entries: { x: { scope: "project", outcome: "remove" } } } ] }), "utf8");
