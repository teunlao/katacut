...
describe("kc install lockfile behavior", () => {
  ...
  it("writes lock by default after apply", async () => {
    ...
    try {
      ...
      await writeFile(join(dir, ".mcp.json"), JSON.stringify({ mcpServers: { a: { type: "http", url: "https://a" } } }), "utf8");
      ...
      await program.parseAsync(["node", "cli", "install", "--client", "claude-code", "--scope", "project", "-c", "katacut.config.jsonc"], { from: "node" });
      cwdSpy.mockRestore();
    ...
    try {
      ...
      vi.doMock("../src/lib/adapters/registry.ts", () => {
        const adapter = {
          ...
          readUser: async () => ({ mcpServers: {} }),
          desiredFromConfig: () => ({ a: { type: "http", url: "https://a" } }),
          applyInstall: async () => { applied++; return { added: 0, updated: 0, removed: 0, failed: 0 }; },
        } as const;
        return { getAdapter: async () => adapter };
      });
      ...
      await program.parseAsync(["node", "cli", "install", "--client", "claude-code", "--scope", "project", "--no-write-lock", "-c", "katacut.config.jsonc"], { from: "node" });
      await expect(stat(join(dir, "katacut.lock.json"))).rejects.toBeTruthy();

      // lockfile-only: write lock, do not apply
      applied = 0;
      await program.parseAsync(["node", "cli", "install", "--client", "claude-code", "--scope", "project", "--lockfile-only", "-c", "katacut.config.jsonc"], { from: "node" });
      ...
      await writeFile(join(dir, "katacut.config.jsonc"), JSON.stringify({ version: "0.1.0", mcp: { b: { transport: "http", url: "https://b" } } }), "utf8");
      applied = 0;
      await program.parseAsync(["node", "cli", "install", "--client", "claude-code", "--scope", "project", "--frozen-lock", "-c", "katacut.config.jsonc"], { from: "node" });
      expect(applied).toBe(0);
