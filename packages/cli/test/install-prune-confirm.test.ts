...
describe("kc install --prune requires --yes", () => {
  it("refuses prune without --yes", async () => {
    ...
    try {
      ...
      await writeFile(join(dir, ".mcp.json"), JSON.stringify({ mcpServers: { extra: { type: "http", url: "https://x" } } }), "utf8");
      ...
      const logs: string[] = [];
      vi.spyOn(console, "error").mockImplementation((s: unknown) => { if (typeof s === "string") logs.push(s); });
      const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(dir);
      await program.parseAsync(["node", "cli", "install", "--client", "claude-code", "--scope", "project", "--prune", "-c", "katacut.config.jsonc"], { from: "node" });
      cwdSpy.mockRestore();
