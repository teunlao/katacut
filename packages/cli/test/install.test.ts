...
describe("kc install (CLI)", () => {
	...
	it("dry-run shows ADD when current is empty; idempotency yields SKIP", async () => {
					...
					for (const [k, v] of Object.entries(c.mcp)) {
						...
						if (v.transport === "http" && typeof v.url === "string") {
							...
							const args = Array.isArray(v.args)
								? (v.args.filter((a) => typeof a === "string") as string[])
								: undefined;
		...
		await program.parseAsync(
			["node", "cli", "install", "--client", "claude-code", "--scope", "project", "--dry-run", "-c", configPath],
			{ from: "node" },
		);
		const jsonLog1 = logSpy.mock.calls
			.map((c) => c[0])
		...
		const jsonLog2 = logSpy.mock.calls
			.map((c) => c[0])
			.reverse()
			.find((s) => typeof s === "string" && String(s).trim().startsWith("["));
	...
	it("apply filters out SKIP and prints summary", async () => {
		...
		vi.doMock("../src/lib/adapters/registry.ts", () => {
			const adapter = {
				...
				checkAvailable: async () => true,
				readProject: async () => ({
					mcpServers: { x: { type: "http", url: "https://different" }, keep: { type: "stdio", command: "echo" } },
				}),
		...
		await program.parseAsync(
			["node", "cli", "install", "--client", "claude-code", "--scope", "project", "-c", configPath],
			{ from: "node" },
		);
		...
		// Summary printed
		const summaryLine =
			(logSpy.mock.calls.map((c) => c[0]).find((s) => typeof s === "string" && String(s).startsWith("Summary:")) as
				| string
				| undefined) ?? "";
