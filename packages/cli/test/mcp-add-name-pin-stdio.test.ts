      ...
      vi.doMock("../src/lib/adapters/registry.ts", () => {
        const adapter = {
          ...
          desiredFromConfig: (c: unknown) => {
            const o = c as { mcp: Record<string, { transport: string; command: string; args: string[] }> };
            const out: Record<string, { type: "stdio"; command: string; args: string[] }> = {} as never;
            for (const [k, v] of Object.entries(o.mcp)) out[k] = { type: "stdio", command: v.command, args: v.args } as any;
            return out;
          },
