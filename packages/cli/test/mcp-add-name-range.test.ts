    ...
    try {
      ...
      vi.doMock("../src/lib/adapters/registry.ts", () => {
        const adapter = {
          ...
          desiredFromConfig: (c: unknown) => {
            const o = c as { mcp: Record<string, { transport: string; url?: string; command?: string; args?: string[] }> };
            const out: Record<string, any> = {};
            for (const [k, v] of Object.entries(o.mcp)) out[k] = v.transport === "http" ? { type: "http", url: v.url } : { type: "stdio", command: v.command, args: v.args };
            return out;
          },
      ...
      const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
        ...
        if (url.includes("/servers/com.example%2Fmy-server/versions/1.9.0")) {
          const payload = { server: { name: "com.example/my-server", remotes: [{ type: "http", url: "https://example/mcp" }] } } as const;
          return new Response(JSON.stringify(payload), { status: 200, headers: { "content-type": "application/json" } });
        }
      ...
      const lock = JSON.parse(await readFile(join(dir, "katacut.lock.json"), "utf8")) as { mcpServers: Record<string, { resolvedVersion?: string }> };
