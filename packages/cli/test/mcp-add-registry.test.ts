    ...
    try {
      ...
      const registryJson = {
        server: {
          ...
          version: "0.4.0",
          remotes: [ { type: "streamable-http", url: "https://example.org/mcp", headers: [ { name: "Authorization", value: "Bearer token" } ] } ],
          packages: null,
        },
        _meta: { published_at: "2025-10-17T00:00:00Z" }
      } as const;
      const fetchMock = vi.fn(async () => new Response(JSON.stringify(registryJson), { status: 200, headers: { "content-type": "application/json" } }));
      ...
      await program.parseAsync(["node","cli","mcp","add","https://registry.modelcontextprotocol.io/v0.1/servers/ai.smithery%2FHint-Services-obsidian-github-mcp/versions/latest"], { from: "node" });
      cwdSpy.mockRestore();
    ...
    try {
      ...
      const registryJson = {
        server: {
          ...
          version: "1.0.0",
          remotes: null,
          packages: [ { registryType: "npm", identifier: "@modelcontextprotocol/server-memory", transport: { type: "stdio" }, runtimeHint: "npx" } ]
        },
        _meta: { published_at: "2025-10-17T00:00:00Z" }
      } as const;
      const fetchMock = vi.fn(async () => new Response(JSON.stringify(registryJson), { status: 200, headers: { "content-type": "application/json" } }));
      ...
      registerMcpCommand(program);
      const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(dir);
      await program.parseAsync(["node","cli","mcp","add","https://registry.modelcontextprotocol.io/v0.1/servers/io.modelcontextprotocol%2Fserver-memory/versions/latest"], { from: "node" });
      cwdSpy.mockRestore();
