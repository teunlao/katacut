		...
		try {
			...
			vi.doMock('../src/lib/adapters/registry.ts', () => {
				const adapter = {
					...
					desiredFromConfig: (c: unknown) => {
						const cfg = c as { mcp: Record<string, { transport: string; url?: string }> };
						const result: Record<string, { type: 'http'; url: string }> = {};
						for (const [k, v] of Object.entries(cfg.mcp)) {
							result[k] = { type: 'http', url: String(v.url ?? '') };
						}
						return result;
					},
			...
			const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
				...
				if (url.includes('/servers?search=filesystem')) {
					const payload = {
						servers: [{ server: { name: 'com.example/filesystem' } }],
						metadata: { count: 1 },
					} as const;
				...
				if (url.includes('/servers/com.example%2Ffilesystem/versions/latest')) {
					const payload = {
						server: { name: 'com.example/filesystem', remotes: [{ type: 'http', url: 'https://api.example.com/mcp' }] },
					} as const;
					return new Response(JSON.stringify(payload), {
						status: 200,
						headers: { 'content-type': 'application/json' },
